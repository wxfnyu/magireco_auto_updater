...
function Get-Platform-Tools {
    Invoke-WebRequest https://dl.google.com/android/repository/platform-tools-latest-windows.zip -OutFile ".\plaform-tools.zip" | out-null
    Expand-Archive .\plaform-tools.zip -DestinationPath .\
}
...
$phonepath = '/data/local/tmp/magireco.apk'
...
if (Get-Command "adb.exe" -ErrorAction SilentlyContinue)
{
    ...
    adb.exe push magireco.apk $phonepath
    "Installing"
    adb.exe shell pm install -i "com.android.vending" -r $phonepath
} else
{
    "No ADB installation detected, downloading"
    Get-Platform-Tools
    "Wating For Phone"
    .\plaform-tools\adb.exe wait-for-device
    .\platform-tools\adb.exe devices
    "Uploading APK to phone"
    .\platform-tools\adb.exe push magireco.apk $phonepath
    "Installing"
    .\platform-tools\adb.exe shell pm install -i "com.android.vending" -r $phonepath
}
